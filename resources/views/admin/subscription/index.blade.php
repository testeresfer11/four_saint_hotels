...
<div class="content-wrapper user-manage-box">
    <div class="top-titlebar d-flex justify-content-between align-items-center pb-2">
        <h2 class="f-20 bold title-main">Subscriptions</h2>
        <div class="add-icon d-flex gap-3">
            <a href="{{route('add-subscription')}}" class="d-flex align-items-center">
            ...
            <div class="card">
                <div class="card-body p-0">
                    <div class="table-responsive">
                        <table id="subscription-table" class="table table-light">
                            <thead>
                                <tr>
                                    <th>Sr.No</th>
                                    <th>Name</th>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        var table = $('#subscription-table').DataTable({
            ...
            ajax: {
                url: '{{ route('subscription-data') }}',  // Fetch data from the server
                data: function(d) {
            ...
            columns: [
                ...
                },
                {
                    ...
                    searchable: false,
                    render: function(data, type, row) {
                         let ticketDetailsUrl = '{{ route('subscription-details', ['subscription_id' => '__ticket_id__']) }}'.replace('__ticket_id__', row.id);
    ...
    $(document).on('click', '.delete-btn', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                window.location.href = '{{ route("delete-subscription", ["subscription_id" => "__subscription_id__"]) }}'.replace("__subscription_id__", subscriptionId);
            }
        });
    });
</script>
